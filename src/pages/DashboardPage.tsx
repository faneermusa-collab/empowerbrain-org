...
import {
  ...
  Calculator, Trophy, Target,
} from 'lucide-react';
import { useAuth } from '@/context/AuthContext';
import { supabase } from '@/lib/supabase';
import type { CourseWithProgress, Commission, CertificateWithCourse, PracticeSession } from '@/types/database';
...
export default function DashboardPage() {
  ...
  const fetchPracticeSessions = async () => {
    const { data } = await supabase
      ...
      .order('created_at', { ascending: false })
      .limit(5);
    setPracticeSessions((data as PracticeSession[]) ?? []);
  };
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard icon={Award} label="Certificates" value={certificates.length.toString()} color="amber" />
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Link to="/calculator" className="card group flex items-center gap-4 p-5 transition-all hover:shadow-lg hover:-translate-y-0.5">
          ...
          <div>
            <p className="text-sm font-semibold text-ink-900">ZMC Trainer</p>
            <p className="text-xs text-ink-500">Practice mental math</p>
          </div>
          <ArrowRight className="ml-auto h-4 w-4 text-ink-300" />
        </Link>
        <Link to="/certificates" className="card group flex items-center gap-4 p-5 transition-all hover:shadow-lg hover:-translate-y-0.5">
          ...
          <div>
            <p className="text-sm font-semibold text-ink-900">Certificates</p>
            <p className="text-xs text-ink-500">{certificates.length} earned</p>
          </div>
          <ArrowRight className="ml-auto h-4 w-4 text-ink-300" />
        </Link>
        <Link to="/franchise" className="card group flex items-center gap-4 p-5 transition-all hover:shadow-lg hover:-translate-y-0.5">
          ...
          <div>
            <p className="text-sm font-semibold text-ink-900">Franchise</p>
            <p className="text-xs text-ink-500">Grow with us</p>
          </div>
          <ArrowRight className="ml-auto h-4 w-4 text-ink-300" />
        </Link>
      </div>
      ...
      <div className="mt-10">
        <div className="flex items-center justify-between">
          ...
          <Link to="/network" className="btn-ghost text-brand-600">
            View Network <ArrowRight className="h-4 w-4 rtl-flip" />
          </Link>
        </div>
      ...
      {practiceSessions.length > 0 && (
        <div className="mt-10">
          <h2 className="text-lg font-semibold text-ink-900">Recent Practice</h2>
          <div className="mt-4 card overflow-hidden">
            <table className="w-full">
              <thead className="bg-ink-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-ink-500 uppercase tracking-wider">Type</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-ink-500 uppercase tracking-wider">Difficulty</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-ink-500 uppercase tracking-wider">Score</th>
              ...
              <tbody className="divide-y divide-ink-100">
                {practiceSessions.map((s) => (
                  <tr key={s.id} className="hover:bg-ink-50/50">
                    <td className="px-4 py-3 text-sm capitalize text-ink-700">{s.session_type}</td>
                    <td className="px-4 py-3 text-sm capitalize text-ink-500">{s.difficulty}</td>
                    <td className="px-4 py-3 text-sm font-semibold text-accent-600">{s.score}/{s.total_questions}</td>
                    <td className="px-4 py-3 text-sm text-ink-500">{new Date(s.created_at).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
